...
public:
	...
	public:
		RTSPClientConnection(RTSPConnection& connection, Environment& env,
				     Callback* callback, const char* rtspURL, int timeout,
				     int rtptransport, int verbosityLevel = 0);
		virtual ~RTSPClientConnection();

		MediaSubsession* getMediaSubSession() { return m_subSession; }
...
public:
	RTSPConnection(Environment& env, Callback* callback, const char* rtspURL,
		       const std::map<std::string, std::string>& opts, int verbosityLevel = 1);
	RTSPConnection(Environment& env, Callback* callback, const char* rtspURL, int timeout = 5,
		       int rtptransport = RTPUDPUNICAST, int verbosityLevel = 1);
	virtual ~RTSPConnection();

	void start(unsigned int delay = 0);
	std::string getUrl() { return m_url; }
	int getRtpTransport() { return m_rtptransport; }
	const char* getFmtpSpropParametersSets() {
		return m_rtspClient->getMediaSubSession()->fmtp_spropparametersets();
	}
	char const* getFmtpSpropvps() const {
		return m_rtspClient->getMediaSubSession()->fmtp_spropvps();
	}
	char const* getFmtpSpropsps() const {
		return m_rtspClient->getMediaSubSession()->fmtp_spropsps();
	}
	char const* getFmtpSproppps() const {
		return m_rtspClient->getMediaSubSession()->fmtp_sproppps();
	}
