...
RTSPConnection::RTSPConnection(Environment& env, Callback* callback, const char* rtspURL,
			       int timeout, int rtptransport, int verbosityLevel)
...
RTSPConnection::RTSPConnection(Environment& env, Callback* callback, const char* rtspURL,
			       const std::map<std::string, std::string>& opts, int verbosityLevel)
  : m_startCallbackTask(NULL),
    m_env(env),
    m_callback(callback),
    m_url(rtspURL),
...
void RTSPConnection::start(unsigned int delay) {
	...
	m_startCallbackTask =
	  m_env.taskScheduler().scheduleDelayedTask(delay * 1000000, TaskstartCallback, this);
}
...
void RTSPConnection::TaskstartCallback() {
	...
	m_rtspClient = new RTSPClientConnection(*this, m_env, m_callback, m_url.c_str(), m_timeout,
						m_rtptransport, m_verbosity);
}
...
RTSPConnection::RTSPClientConnection::RTSPClientConnection(RTSPConnection& connection,
							   Environment& env, Callback* callback,
							   const char* rtspURL, int timeout,
							   int rtptransport, int verbosityLevel)
  : RTSPClientConstrutor(env, rtspURL, verbosityLevel, NULL,
...
void RTSPConnection::RTSPClientConnection::setNptstartTime() {
	...
	while (
	  std::getline(std::getline(std::getline(is, querystring, '?'), key, '='), value, '&')) {
		opts[key] = value;
	}
	...
	if (m_subSessionIter == NULL) {
		...
		if (m_subSession != NULL) {
			...
			if (fVerbosityLevel > 1) {
				envir()
				  << "****************************************************************************************\n";
				envir() << " nptTime is given read video from it :: m_nptStartTime "
					<< m_nptStartTime << "\n";
				envir()
				  << " clockstarttime is given read video from it :: m_clockStartTime "
				  << m_clockStartTime.c_str() << "\n";
			}
			if (m_clockStartTime != "") {
				m_playforinit = 1;
...
void RTSPConnection::RTSPClientConnection::continueAfterDESCRIBE(int resultCode,
								 char* resultString) {
	if (resultCode != 0) {
...
void RTSPConnection::RTSPClientConnection::continueAfterSETUP(int resultCode, char* resultString) {
	if (resultCode != 0) {
		...
		if (sink == NULL) {
			envir() << "Failed to create sink for \"" << m_subSession->mediumName()
				<< "/" << m_subSession->codecName()
				<< "\" subsession error: " << envir().getResultMsg() << "\n";
			m_callback->onError(m_connection, envir().getResultMsg());
		} else if (m_callback->onNewSession(sink->name(), m_subSession->mediumName(),
						    m_subSession->codecName(),
...
void RTSPConnection::RTSPClientConnection::continueAfterPAUSE(int resultCode, char* resultString) {
	if (resultCode != 0) {
		...
		if (fVerbosityLevel > 1) {
			...
			envir() << " continueAfterPAUSE m_clockStartTime "
				<< m_clockStartTime.c_str() << "\n";
		}
		m_playforinit = 0;
		if (m_clockStartTime != "") {
...
void RTSPConnection::RTSPClientConnection::continueAfterTEARDOWN(int resultCode,
								 char* resultString) {
	if (resultCode != 0) {
