...
ExtractRtspUrl(const std::string& url) {
	std::regex pattern(
	...
	if (std::regex_search(url, matches, pattern)) {
		...
		if (matches[2].matched && matches[3].matched) {
			return std::make_tuple(matches[2].str(),
					       matches[3].str(), rtspUri);
		} else {
...
obs_properties* RtspSource::GetProperties() {
	...
	obs_property_t* prop =
	  obs_properties_add_text(props, "url", "RTSP URL", OBS_TEXT_DEFAULT);
	obs_property_set_long_description(prop,
					  "Specify the RTSP URL to play.");
...
bool RtspSource::OnApplyBtnClicked(obs_properties_t* props,
	...
	if (rtsp.empty()) {
		blog(LOG_ERROR, "Current RTSP url(%s) is invalidate",
		     url.c_str());
		return false;
	}
...
// override methods
void RtspSource::OnSessionStarted(const char* id, const char* media,
...
void RtspSource::OnData(unsigned char* buffer, ssize_t size,
			struct timeval time) {
  ...
  // decode h264 frame here
	...
	obs_source_frame* frame =
	  obs_source_frame_create(VIDEO_FORMAT_I420, 1920, 1080);
	if (frame == nullptr) {
		return;
	}
